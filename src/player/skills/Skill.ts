...
export abstract class Skill {
    ...
    use(player: Player, scene: THREE.Scene, world: CANNON.World): boolean {
        ...
        if (!this.canUse(player)) {
            this.uiManager.displayInsufficientTPWarning();
        ...
        // Execute skill
        this.executeAndCallCompletionCallback(player, scene, world);
    ...
    /**
     * Execute the sill and call the onCompleted callback when done.
     */
    private executeAndCallCompletionCallback(player: Player, scene: THREE.Scene, world: CANNON.World) {
