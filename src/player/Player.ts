...
export class Player extends BaseMesh {
    ...
    // longest skill effect is RecoverySkill at 1.5 s (BASE_DURATION). This value is set
    ...
    get collectionBonusSkillCooldownReduction(): number {
        return this.cardCollection.isAlbumComplete(Album.C002) ? 0.10 : 0;
    }
    ...
    tryIncrementWeaponTech(dropRateFactor: number) {
        ...
        const levelTechCapReached = weaponTechPoints >= this.tierManager.getTechCapForLevel(this.level);
    ...
    tryIncrementSkillTech(type: SkillTechType): void {
        ...
        const levelTechCapReached = skillTechPoints >= this.tierManager.getTechCapForLevel(this.level);
    ...
    getSkillTier(type: SkillTechType): Tier {
        return this.tierManager.getSkillTierForTech(this.skillTech[type]);
    }
    ...
    private setupAnimations() {
        ...
        const gltf = container.resolve(AssetManager).get('models/main_character.glb');
        ...
        if (animations && animations.length > 0) {
            ...
            const idleClip = getClip(PlayerActionType.Idle);
            const runOneHandedClip = getClip(PlayerActionType.RunOneHanded);
            ...
            const powerUpClip = getClip(PlayerActionType.PowerUp);
            ...
            if (powerUpClip) {
                ...
                action.loop = THREE.LoopOnce;
                action.clampWhenFinished = true;
                this.actions[PlayerActionType.PowerUp] = action;
            }
        ...
        // Start Idle
        this.fadeToAction(PlayerActionType.Idle, 0.0);
    }

    private fadeToAction(actionType: PlayerActionType, duration: number) {
    ...
    private updateAnimations() {
        ...
        const powerUpAction = this.actions[PlayerActionType.PowerUp];
        if (this.isLevelingUp && powerUpAction && powerUpAction.isRunning()) {
            return;
        }

        // High priority: Take Hit
        const takeHitAction = this.actions[PlayerActionType.TakeHit];
        ...
        // Run / Idle
        const isMoving = this.inputManager.getMovementVector().length() > 0.1;
        if (isMoving) {
            const action = this.weapon.weaponType !== WeaponType.HAMMER ? PlayerActionType.RunOneHanded : PlayerActionType.RunTwoHanded;
    ...
    update(dt: number, isNearInteractive: boolean = false, isAnyMenuOpen: boolean = false) {
        ...
        if (this.inputManager.isBlockJustPressed()) {
            this.startBlock();
        }
        ...
        if (this.inputManager.isAttackReleased()) this.attackLockedUntilRelease = false;
    ...
    private handleDash(dt: number): boolean {
        ...
        this.fadeToAction(PlayerActionType.Dash, 0.0);
        this.dashTimer += dt;
    ...
    private handleCharging(dt: number): boolean {
        ...
        if (this.inputManager.isAttackReleased()) {
    ...
    private handleMovement(dt: number, isNearInteractive: boolean) {
        ...
        if (!this.weapon.isAttacking) {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                perimeterRay.intersectWorld(this.physicsWorld, { mode: CANNON.Ray.CLOSEST, result: result, skipBackfaces: true });
                perimeterRayResults.push(result);
            }

            const centerRayResult = new CANNON.RaycastResult();

            centerRay.intersectWorld(this.physicsWorld, { mode: CANNON.Ray.CLOSEST, result: centerRayResult, skipBackfaces: true });
            ...
            if (this.inputManager.isJumpPressed() || Math.abs(inputVector.x) > Number.EPSILON || Math.abs(inputVector.y) > Number.EPSILON) {
                this.body.type = CANNON.Body.DYNAMIC;
            }

            if (this.inputManager.isJumpPressed() && this.isGrounded && !isNearInteractive && this.jumpCooldownTimer <= 0) {
    ...
    private handleCombat(dt: number) {
        ...
        if (this.inputManager.isAttackJustPressed()) this.chargeDelayTimer = 0;
    ...
    private startBlock(): void {
        ...
        this.fadeToAction(PlayerActionType.Idle, 0.1);
    ...
    private handleSkills(): void {
        ...
        if (this.inputManager.isSkill1JustPressed() && this.isSkillUnlocked(0)) {
            this.useSkill(0); // Laser Beam
        } else if (this.inputManager.isSkill2JustPressed() && this.isSkillUnlocked(1)) {
            this.useSkill(1); // Healing
        } else if (this.inputManager.isSkill3JustPressed() && this.isSkillUnlocked(2)) {
    ...
    private useSkill(skillIndex: number): void {
        ...
        // Execute the skill
        if (skill.use(this, this.scene, this.physicsWorld)) {
    ...
    syncPosition() {
        ...
        groundRay.intersectWorld(this.physicsWorld, { mode: CANNON.Ray.CLOSEST, result: groundRayResult, skipBackfaces: true });
    ...
    private updateFootstepAudio(dt: number, isMoving: boolean): void {
        ...
        if (this.footstepTimer <= 0) {
            this.audioManager.playFootstep('player');
    ...
    takeDamage(amount: number, sourcePos?: CANNON.Vec3, isCriticalHit: boolean = false): void {
        ...
        this.audioManager.playDamage('player');
        ...
        this.fadeToAction(PlayerActionType.TakeHit, 0.05);
    ...
    private die(): void {
        ...
        this.audioManager.playDeath('player');

        this.fadeToAction(PlayerActionType.Death, 0.1);
    ...
    private executeDashAttack() {
        ...
        this.dashTimer = 0;
        this.dashHitEnemies.clear();
        this.audioManager.playAttack('player', true);
    ...
    private levelUp(): void {
        ...
        this.fadeToAction(PlayerActionType.PowerUp, 0.1);
