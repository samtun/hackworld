...
export class InventoryManager {
    ...
    private generateStatsHTML(player: Player): string {
        ...
        const dualBladeTechColor = player.tech[WeaponType.DUAL_BLADE] == techCap ? cappedTechColor : defaultTechColor;
        const lanceTechColor = player.tech[WeaponType.LANCE] == techCap ? cappedTechColor : defaultTechColor;
        const hammerTechColor = player.tech[WeaponType.HAMMER] == techCap ? cappedTechColor : defaultTechColor;
        ...
        const techHTML = `${sep}
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap: 4px">
                <div style="display:flex; align-items:center; gap:4px;">${getWeaponIcon(WeaponType.SWORD)}<div><div style="font-size:13px; color:#aaa;">Sword</div><div style="color:${swordTechColor}">${player.tech[WeaponType.SWORD]} | <span style="font-style:italic;color:#BBB;">${weaponLevelChar(player.tech[WeaponType.SWORD])}</span></div></div></div>
